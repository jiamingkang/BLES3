...
double DeltaThetaMax = 0.1;		/*Maximum change in fibre angel per iteration*/
    int ittcounter=0;       // counter for itteration(to make the code run deepest descent method)
    int countt=0;
...
for(p=0;p<Numlsf;p++)
{
    ...
    *DeltaT=0.0;
      ScaleDeltaTheta(elemX, elemY, ElemThetaStn, DeltaTheta, DeltaThetaMax, itt, CompA, Number, ElemStat, ElemStatb, Numlsf, NodeThetaStn, NodeDTL, DeltaT, Ntot_new, Vnorm, &SensMaxOld,ittcounter);
...
*/
    ...
    //if (itt>1 && ittcounter<5)    // this is for another method
    if (itt>1)
    {
