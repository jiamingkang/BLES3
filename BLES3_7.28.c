...
double DeltaThetaMax = 0.1;		/*Maximum change in fibre angel per iteration*/
...
do {
	...
	NodeStat = calloc(NumNodes,sizeof(short));	/*Node status array defined using dynamic memeory allocation*/
    ...
    bptr = malloc(NumElem * sizeof(Bseg));
...
for(p=0;p<Numlsf;p++)
{
	...
	IFG_Status(h,elemX,elemY,Number,&Ntot,NumElem,NodeX,NodeY,Nodes2,NumNodes,NodeCoord,lsf2b,NodeStat,ElemStatb,auxNode,ftv_lim,itt,hxz,hz, AuxNodes, bptr, na_conn_ind, na_conn, NumAux, NumBound);
    
    Ntot_new=Ntot_new+*NumAux;
...
for(p=0;p<Numlsf;p++)
{
    ...
    // Get the ture sensitivity at each single node point
    
    NodeSen(NumNodes, DTL, ElemStatb,  NodesenTemp, ElemThetaStn);
     int kk;
    ...
    GPsensitivity( elemX, elemY, Number,  NumNodes, NodeCoord, h, hxz,  Ntot_new, AuxNodes, NodeDTL, NodesenTemp, GPsens);
    ...
    NodeAuxsens( elemX, elemY, Number, NumNodes,  NodeCoord, h,  hxz, Ntot_new, AuxNodes,GPsens,Nodesenpp);
...
for(p=0;p<Numlsf;p++)
{
    ...
    Vnorm = malloc(Ntot_new*sizeof(double));
    
    for(i=0; i<Ntot_new; i++)
    {
    ...
    /*  test the MatrixMultople function
    double *aaa;
    double *bbb;
    double *res;
    res=malloc(4*sizeof(double));
    aaa=malloc(16*sizeof(double));
    bbb=malloc(16*sizeof(double));
    ...
    MatrixMultiple(aaa, bbb, res, 8, 2, 2, 8);
    for(i=0;i<4;i++)
    {
        printf("hahahhha: %f", res[i]);
    }
     */
   ...
   // ScaleDeltaTheta(elemX, elemY, ElemThetaStn, DeltaTheta, DeltaThetaMax, itt, CompA, Number, ElemStat, ElemStatb, Numlsf, NodeThetaStn, NodeDTL, DeltaT, Ntot_new, Vnorm );
    ...
    Vext(NodeX, NodeY,Nodes2,NodeStat,Vnorm,lsf2,active,NumAct,NumNodes, Ntot_new,NodeCoord, h, AuxNodes, -1 , dt,elemX,hxz);
    Vext(NodeX, NodeY,Nodes2,NodeStat,Vnorm,lsf2,active,NumAct,NumNodes, Ntot_new,NodeCoord, h, AuxNodes, 1 , dt,elemX,hxz);
    ...
    *DeltaT=0.0;
      ScaleDeltaTheta(elemX, elemY, ElemThetaStn, DeltaTheta, DeltaThetaMax, itt, CompA, Number, ElemStat, ElemStatb, Numlsf, NodeThetaStn, NodeDTL, DeltaT, Ntot_new, Vnorm, &SensMaxOld);
...
for(p=0;p<Numlsf;p++)
{
    ...
    // This part is to predict the value of compliance
    
    
    double Eenergy_old[elemX*elemY];
    double pred_comp[200];
    pred_comp[0]=0.0;
    double Eenergy_total,Eenergy_total2,Eenergy_total3;
    
    if(itt>0)
    {
        //get the new theta
        double *theta_new, *Eenergy_new;
        
        theta_new=malloc(elemX*elemY*sizeof(double));
        Eenergy_new=malloc(elemX*elemY*sizeof(double));
        
        
        Eenergy_total=0.0;
        Eenergy_total2=0.0;
        Eenergy_total3=0.0;
        //ThetaUpdate4(theta_new, elemX, elemY, Number, lsf2, lsf2b, Numlsf, h, hxz);
        	ThetaUpdate3(theta_new, elemX, elemY, Number, lsf2, h, hxz);
        //ThetaUpdate4(theta_new, elemX, elemY, Number, lsf2, lsf2b, 1, h, hxz);
        //  Eenergy[j] is the compliance of each elment.
        
        for (i=0; i<elemX*elemY; i++) {
            
            Eenergy_new[i]=ElemThetaStn[i]*(theta_new[i]-theta[i])+Eenergy[i];
           //Eenergy_new[i]=ElemThetaStn[i]*(theta_new[i]-theta_old[i])+Eenergy_old[i];
            Eenergy_total+=Eenergy_new[i];
            //printf("\n Theta change of element %d is %f", i, 180*(theta_new[i]-theta[i])/3.14159);
            printf("\n %f", i, 180*(theta_new[i]-theta[i])/3.14159);
        }
        printf("\npredicted compliance is Energy: %f", Eenergy_total);
        for (i=0; i<elemX*elemY; i++) {
            //Eenergy_new[i]=ElemThetaStn[i]*(theta_new[i]-theta[i])+Eenergy[i];
            //Eenergy_new[i]=ElemThetaStn[i]*(theta_new[i]-theta_old[i])+Eenergy_old[i];
            //Eenergy_total2+=Eenergy_new[i]*ElemStat[i];
            //Eenergy_total3+=Eenergy[i]*ElemStat[i];
        }
        //printf("\npredicted compliance at intersected element: %f", Eenergy_total2);
        //printf("\ncompliance at intersected element of this itt: %f", Eenergy_total3);
        
        
        pred_comp[itt]=Eenergy_total;
        free(Eenergy_new);
        free(theta_new);
        // took from qianmian
        if(itt>0)
    ...
    //////////////-------------------------------
    // compare the predicted and the current compliance, if predicted is larger, then low down the change of lsf value,(half the maxlsf change, 0.1-->0.05)
    //  then check it again,
    // if in 10 sequence itt, it is still ok, then larger the max change
    
    /*
    if (Eenergy_total2-dist>0.00000000001) {
        
        *DeltaT=0.5*(*DeltaT);
        
        for(i=0;i<(Numlsf*NumNodes);i++)
        {
            lsf2[i]-= *DeltaT*NodesenTemp[i];
            
           // printf("delta %d lsf: %f, grad: %f\n", i,(*DeltaT)*Deltalsf2[i]);
        }
        for(i=0;i<(elemX*elemY);i++)
        {
            //	SensB[itt] += DeltaTheta[i];
    ...
    if (itt==50) {
        //DeltaThetaMax=2*DeltaThetaMax;
    }
    if (itt==60) {
        //DeltaThetaMax=4*DeltaThetaMax;
    }
    
    countt++;     // to count how many times didn't change the stepsize
    
    if (Eenergy_total-dist>0.0000001&&itt>0) {
        //scale_factor=0.5;
        countt =0;
