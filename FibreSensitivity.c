...
void ScaleDeltaTheta(int elemX, int elemY, double *ElemThetaStn , double *DeltaTheta, double DeltaThetaMax, int itt, double CompA[itt],  Elem Number[elemX][elemY], short *ElemStat, short *ElemStatb, int Numlsf, double *NodeThetaStn, double *NodeDTL, double *DeltaT, int Ntot_new , double *Vnorm,double *SensMaxOld, int ittcounter)
{
	FILE *outfile;	/*File varible for output files*/
	char plotname[40];	/*variable to change names of plotting output files*/
	int i,j,num,p,k;
   ...
   // }
    ...
    DeltaThetaMax =0.1;       // this should be commment out when use level set method, only work for
    ...
    *DeltaT = Ctemp*DeltaThetaMax/(fabs(SensMax));		/*Get desired change in theta.*/
    ...
    if (fabs(SensMax)-0.3<=0)
    {
       // *DeltaT=0.3;
    
    }
    if (ittcounter>10)
    {   printf("HAHAHAHAHAAHAHAHAH");
       // *DeltaT = Ctemp*DeltaThetaMax/(fabs(SensMax)+1);
